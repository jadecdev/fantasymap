...
- **`renderListPanels(domKey)`**: rebuilds legend + transcript + header texts based on the dominant region. `domKey === null` → "Régions de France" with 10 region rows. `domKey === "33"` → "Peuples du 33" with 30 city rows. Called only when `currentDominant` actually changes (guard against pan spam).
- **Selector**: a `#region-dropdown` containing a `.region-dropdown-trigger` button that toggles a `.region-dropdown-menu` with 11 `.region-dropdown-item` options (1 « Vue France » + 10 departments). `data-region="france"` calls `flyToBounds(FRANCE_BOUNDS)`. Department items call `flyTo(region.center, region.zoom)`. The trigger label and the `.active` class on the items are set by `renderListPanels` based on the current dominant region — never persisted.
